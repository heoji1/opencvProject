...
int main() {
    ...
    for (int i = 0; i < n; i++) {
        ptl_x[i] = round(srcPoints[i].x);
        ptl_y[i] = round(srcPoints[i].y);
        ptr_x[i] = round(dstPoints[i].x);
        ptr_y[i] = round(dstPoints[i].y);
    }
	...
	namedWindow("Case1 result");
	imshow("Case1 result", I_f);

	I_f.convertTo(I_f, CV_8UC3, 255.0);
	imwrite("result_case1.png", I_f);
